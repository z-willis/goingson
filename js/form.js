   ...
   createAccountPass.focus(function(){
    ...
    repeatPass.change(function(){        
        if($(this).val() == createAccountPass.val()){
            $(this).next().text("Passwords match").animate({
                opacity: 1
            }, 1000).animate({
                opacity: 0
            }, 1000);
        }else{
            $(this).next().text("Passwords don't match").animate({
                opacity: 1
            }, 1000).animate({
                opacity: 0
            }, 1000);
...
function checkAvailability() {
    jQuery.ajax({
        ...
        success:function(data){
            if(data > 0){
                createAccountUser.next().next().text("Username already exists").animate({
                    opacity: 1
                }, 1000).animate({
                    opacity: 0
                }, 1000);
            }else{
                createAccountUser.next().next().text("Username available").animate({
                    opacity: 1
                }, 1000).animate({
                    opacity: 0
                }, 1000);
